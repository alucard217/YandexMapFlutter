      ...
      PlacemarkMapObject(
        ...
        onTap: (self, point) {
          showDialog(
            context: context,
            builder: (_) => const AlertDialog(
              title: Text('Парк Президентский'),
              content: Text('Красивое место с большим пространством.'),
            ),
          );
        },
      ),
...
Widget _buildBookingDialog(BuildContext context) {
  ...
  return AlertDialog(
    title: const Text('Запись на площадку'),
    content: Column(
      ...
      children: [
        ...
        TextField(
          ...
          decoration: const InputDecoration(
            labelText: 'Время (например, 17:00)',
          ),
        ),
      ],
    ),
    actions: [
      TextButton(
        onPressed: () {
          ...
          print('Пользователь: $name, Время: $time');
